...
class Login extends Component {
  ...
  login(){
    ...
    if(userData.email === "" || userData.password === "" || userData.repassword === "" ){
      ...
      this.setState({ emailKey: false, passwordKey: false });
      axios.post(`${Root.HOST}auth/login` , this.state.userData).then(rdata => {
            ...
            <div className="col-md-12 mx-auto mb-9">
              ...
              <div className="form start-form">
                <div className="upload start-form-pane" style = {{width : "20%"}}>
                  ...
                  <button onClick = {() => this.login()} className="btn btn-white">Submit</button>
                </div>
              </div>
            </div>
        </div>
